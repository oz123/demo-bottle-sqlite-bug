...
class Bottle(object):
    ...
    def _mount_wsgi(self, prefix, wsgiapp, **options):
        ...
        def mountpoint_wrapper():
            try:
                ...
                body = wsgiapp(request.environ, start_response)
        ...
        options.setdefault('mountpoint', {'prefix': prefix, 'target': wsgiapp})
    ...
    def _mount_app(self, prefix, mountedapp, **options):
        if mountedapp in self._mounts or '_mount.app' in mountedapp.config:
        ...
        self._mounts.append(mountedapp)
        mountedapp.config['_mount.prefix'] = prefix
        mountedapp.config['_mount.app'] = self
        for route in mountedapp.routes:
            route.rule = prefix + route.rule.lstrip('/')
            self.add_route(route)

    def mount(self, prefix, mountedapp, **options):
        ...
        if isinstance(mountedapp, Bottle):
            return self._mount_app(prefix, mountedapp, **options)
        else:
            return self._mount_wsgi(prefix, mountedapp, **options)
