...
class SQLitePlugin(object):
    ...
    def apply(self, callback, route):
        ...
        # The reason it fails, is because _callback is no
        # longer the original function.
        # this is solved in python3.3 with
        # inspect.signature(callable, *, follow_wrapped=True)
        # in earlier versions of python you would have to
        # @wraps your original function and check if the callback
        # has __wrapped__. This will only work for 1 level, if you
        # do :
        #    @app.route('/secret/:item')
        #    @valid_user()
        #    @has_permission()
...
class SQLitePluginFixed(object):
    ''' This plugin passes an sqlite3 database handle to route callbacks
    that accept a `db` keyword argument. If a callback does not expect
    such a parameter, no connection is made. You can override the database
    settings on a per-route basis. '''
